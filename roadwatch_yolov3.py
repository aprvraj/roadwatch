...
# if the frame is an empty value, then the end of the video has been reached
	if frame is None:
		print("=============================================")
...
# depending on the frame size, we set the minimum radius and the maximum number of frames for centroid tracker
...
# т.к. все выше-обработка одного кадра, то now it is necessary to increase the number of frames
# and update the counter
	totalFrames += 1

# The graph is displayed on the screen at the end of the program's work
plt.show()

# We free up memory under variable
if writer is not None:
	writer.release()
